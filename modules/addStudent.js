...
conditionStep.on("callback_query", async (ctx) => {
    try {
        ...
        /* Средний чек */
        let averageCheck
        averageCheck = parseInt(wizardDate.paymentAmount) / parseInt(dueDate)
        ...
        D.setMonth(D.getMonth() + parseInt(dueDate));
...
const studentsScene = new WizardScene('StudentWizard', startStep, paymentAmountText, studentName, studentClass, parentName,paymentAmount,
    prepaymentSum, paymentReceived, startLearning, conditionStep, studentNameRepeat, surchargeSum)
